...
async function runStep1_ClassifyText(
  ...
  const { content: step1Content, tokens } = await callGPT({
    effort: 'none',
    systemPrompt: `Ты — B2B-аналитик для ПРОДАКШН-СТУДИИ с Дальнего Востока России. Студия ПРОДАЁТ ПРОДАКШН-УСЛУГИ КИТАЙСКИМ КОМПАНИЯМ: съёмка коммерческих роликов, документалок, бренд-фильмов, дизайн-пакетов — всё НА ЗАКАЗ.

Контекст: проведён опрос ${sampled.length} китайских студентов на Дальнем Востоке России. Открытый вопрос: "Какой продукт/контент креативных индустрий Дальнего Востока России вы хотели бы видеть на китайском рынке?". Ответы на китайском.
...
- Если про туризм → аргумент для китайских турагентств: "закажите у нас промо-ролик о Дальнем Востоке России"
...
async function runStep2_BuildPersonas(
  ...
  const { content: step2Content, tokens } = await callGPT({
    effort: 'high',
    systemPrompt: `Ты — B2B-стратег для ПРОДАКШН-СТУДИИ с Дальнего Востока России. Бизнес-модель студии: они ПРОДАЮТ ПРОДАКШН-УСЛУГИ КИТАЙСКИМ КОМПАНИЯМ (снимают видео, документалки, бренд-фильмы, делают дизайн — всё НА ЗАКАЗ).
...
Тебе даны ПРЕДВЫЧИСЛЕННЫЕ статистики опроса ${totalResponses} китайских студентов на Дальнем Востоке России: частотные таблицы, кросс-табуляции (хи-квадрат, Cramér's V), корреляции.
  ...
  "personas": [
    {
      ...
      "vlk_attitude": "отношение к контенту/продуктам креативных индустрий Дальнего Востока России",
...
async function runStep3_DemandMatrix(
  ...
  const { content: step3Content, tokens } = await callGPT({
    effort: 'medium',
    systemPrompt: `Ты — B2B-аналитик для ПРОДАКШН-СТУДИИ с Дальнего Востока России.
...
— Локейшн-сервисы (помощь китайским съёмочным группам на Дальнем Востоке России)
...
КЛИЕНТ: продакшн-студия с Дальнего Востока России. Они ПРОДАЮТ УСЛУГИ КИТАЙСКИМ КОМПАНИЯМ: снимают коммерческие ролики, документалки, бренд-фильмы, делают дизайн — всё НА ЗАКАЗ. Они НЕ блогеры, НЕ инфлюенсеры, НЕ продают курсы.
...
- "Предложите китайским турагентствам снять промо-серию о Дальнем Востоке России (3×5 мин) — 67% респондентов интересуются туризмом на Дальнем Востоке"
